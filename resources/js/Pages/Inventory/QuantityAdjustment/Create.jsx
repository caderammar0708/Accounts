...
export default function CreateAdjustment({ items, accounts, existingReasons = [], nextReference }) {
    ...
    const submit = (e, close = false, pinOverride = null) => {
        ...
        console.log("Submitting inventory adjustment...", { close, pinOverride, data });
        ...
        post(route('inventory-adjustment.store', { action: close ? 'new' : 'save' }), {
            onSuccess: (page) => {
                ...
                showToast('success', page?.props?.flash?.success || 'Inventory quantity adjustment saved successfully.');
                if (close) {
                    setData({
                        ...data,
            ...
            onError: (errs) => {
                console.error("Inventory adjustment save failed:", errs);
                const firstError = Object.values(errs)[0];
                const msg = typeof firstError === 'string' ? firstError : 'Failed to save adjustment. Please check the form errors.';
                showToast('error', msg);
            }
        });
    };
